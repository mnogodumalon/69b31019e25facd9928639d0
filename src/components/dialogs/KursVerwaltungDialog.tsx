...
interface KursVerwaltungDialogProps {
  ...
  defaultValues?: KursVerwaltung['fields'];
  kursleiter_verwaltungList: KursleiterVerwaltung[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function KursVerwaltungDialog({ open, onClose, onSubmit, defaultValues, kursleiter_verwaltungList, enablePhotoScan = false, enablePhotoLocation = true }: KursVerwaltungDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'kurs_verwaltung');
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="kursleiter" entity="Kursleiter-Verwaltung">\n${JSON.stringify(kursleiter_verwaltungList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "kursgebuehr": number | null, // Kursgebühr (€)\n  "kursname": string | null, // Kursname\n  "kursbeschreibung": string | null, // Kursbeschreibung\n  "yoga_stil_kurs": LookupValue | null, // Yoga-Stil (select one key: "vinyasa" | "hatha" | "yin" | "ashtanga" | "kundalini" | "restorative" | "power" | "bikram") mapping: vinyasa=Vinyasa Yoga, hatha=Hatha Yoga, yin=Yin Yoga, ashtanga=Ashtanga Yoga, kundalini=Kundalini Yoga, restorative=Restorative Yoga, power=Power Yoga, bikram=Bikram Yoga\n  "niveau": LookupValue | null, // Kursniveau (select one key: "anfaenger" | "mittelstufe" | "fortgeschrittene" | "alle") mapping: anfaenger=Anfänger, mittelstufe=Mittelstufe, fortgeschrittene=Fortgeschrittene, alle=Alle Niveaus\n  "kursleiter": string | null, // Display name from Kursleiter-Verwaltung (see <available-records>)\n  "wochentag": LookupValue | null, // Wochentag (select one key: "montag" | "dienstag" | "mittwoch" | "donnerstag" | "freitag" | "samstag" | "sonntag") mapping: montag=Montag, dienstag=Dienstag, mittwoch=Mittwoch, donnerstag=Donnerstag, freitag=Freitag, samstag=Samstag, sonntag=Sonntag\n  "startzeit": string | null, // Startzeit\n  "endzeit": string | null, // Endzeit\n  "startdatum": string | null, // YYYY-MM-DD\n  "enddatum": string | null, // YYYY-MM-DD\n  "ort": string | null, // Ort / Raum\n  "max_teilnehmer": number | null, // Maximale Teilnehmerzahl\n}`;
      ...
      setFields(prev => {
        ...
        if (kursleiterName) {
          const kursleiterMatch = kursleiter_verwaltungList.find(r => matchName(kursleiterName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (kursleiterMatch) merged['kursleiter'] = createRecordUrl(APP_IDS.KURSLEITER_VERWALTUNG, kursleiterMatch.record_id);
        }
  ...
  const DIALOG_INTENT = defaultValues ? 'Kurs-Verwaltung bearbeiten' : 'Kurs-Verwaltung hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="kursbeschreibung">Kursbeschreibung</Label>
            <Textarea
              id="kursbeschreibung"
              value={fields.kursbeschreibung ?? ''}
              onChange={e => setFields(f => ({ ...f, kursbeschreibung: e.target.value }))}
              rows={3}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="yoga_stil_kurs">Yoga-Stil</Label>
            <Select
              value={lookupKey(fields.yoga_stil_kurs) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, yoga_stil_kurs: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="yoga_stil_kurs"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="niveau">Kursniveau</Label>
            <Select
          ...
          <div className="space-y-2">
            <Label htmlFor="kursleiter">Kursleiter</Label>
            <Select
              value={extractRecordId(fields.kursleiter) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, kursleiter: v === 'none' ? undefined : createRecordUrl(APP_IDS.KURSLEITER_VERWALTUNG, v) }))}
            >
              <SelectTrigger id="kursleiter"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {kursleiter_verwaltungList.map(r => (
          ...
          <div className="space-y-2">
            ...
            <Select
              value={lookupKey(fields.wochentag) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, wochentag: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="wochentag"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
