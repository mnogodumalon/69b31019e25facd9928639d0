...
export function TeilnehmerAnmeldungDialog({ open, onClose, onSubmit, defaultValues, kurs_verwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: TeilnehmerAnmeldungDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="kurs" entity="Kurs-Verwaltung">\n${JSON.stringify(kurs_verwaltungList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        if (kursName) {
          const kursMatch = kurs_verwaltungList.find(r => matchName(kursName!, [String(r.fields.kursname ?? '')]));
          if (kursMatch) merged['kurs'] = createRecordUrl(APP_IDS.KURS_VERWALTUNG, kursMatch.record_id);
        }
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="gesundheitshinweise">Gesundheitliche Hinweise / Anmerkungen</Label>
            <Textarea
          ...
          <div className="space-y-2">
            <Label htmlFor="einverstaendnis">Ich stimme der Teilnahme und den Kursbedingungen zu</Label>
            <div className="flex items-center gap-2 pt-1">
              <Checkbox
                id="einverstaendnis"
                checked={!!fields.einverstaendnis}
                onCheckedChange={(v) => setFields(f => ({ ...f, einverstaendnis: !!v }))}
              />
              <Label htmlFor="einverstaendnis" className="font-normal">Ich stimme der Teilnahme und den Kursbedingungen zu</Label>
            </div>
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="teilnehmer_email">E-Mail-Adresse</Label>
            <Input
              id="teilnehmer_email"
              type="email"
          ...
          <div className="space-y-2">
            <Label htmlFor="teilnehmer_telefon">Telefonnummer</Label>
            <Input
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, kurs: v === 'none' ? undefined : createRecordUrl(APP_IDS.KURS_VERWALTUNG, v) }))}
            >
              <SelectTrigger id="kurs"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {kurs_verwaltungList.map(r => (
          ...
          <div className="space-y-2">
            <Label htmlFor="erfahrungslevel">Dein Erfahrungslevel</Label>
            <Select
              value={lookupKey(fields.erfahrungslevel) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, erfahrungslevel: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="erfahrungslevel"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
