...
interface KursleiterVerwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: KursleiterVerwaltung['fields']) => Promise<void>;
  defaultValues?: KursleiterVerwaltung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function KursleiterVerwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: KursleiterVerwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<KursleiterVerwaltung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'kursleiter_verwaltung');
      await onSubmit(clean as KursleiterVerwaltung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<KursleiterVerwaltung['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Kursleiter-Verwaltung bearbeiten' : 'Kursleiter-Verwaltung hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="yoga_stil">Yoga-Spezialisierungen</Label>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_hatha"
                  checked={lookupKeys(fields.yoga_stil).includes('hatha')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                      const next = checked ? [...current, 'hatha'] : current.filter(k => k !== 'hatha');
                      return { ...f, yoga_stil: next.length ? next as any : undefined };
                    });
                  }}
                />
                <Label htmlFor="yoga_stil_hatha" className="font-normal">Hatha Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_vinyasa"
                  checked={lookupKeys(fields.yoga_stil).includes('vinyasa')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                ...
                />
                <Label htmlFor="yoga_stil_vinyasa" className="font-normal">Vinyasa Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_yin"
                  checked={lookupKeys(fields.yoga_stil).includes('yin')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                ...
                />
                <Label htmlFor="yoga_stil_yin" className="font-normal">Yin Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_ashtanga"
                  checked={lookupKeys(fields.yoga_stil).includes('ashtanga')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                ...
                />
                <Label htmlFor="yoga_stil_ashtanga" className="font-normal">Ashtanga Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_kundalini"
                  checked={lookupKeys(fields.yoga_stil).includes('kundalini')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                ...
                />
                <Label htmlFor="yoga_stil_kundalini" className="font-normal">Kundalini Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_restorative"
                  checked={lookupKeys(fields.yoga_stil).includes('restorative')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                ...
                />
                <Label htmlFor="yoga_stil_restorative" className="font-normal">Restorative Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_power"
                  checked={lookupKeys(fields.yoga_stil).includes('power')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                ...
                />
                <Label htmlFor="yoga_stil_power" className="font-normal">Power Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="yoga_stil_bikram"
                  checked={lookupKeys(fields.yoga_stil).includes('bikram')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.yoga_stil);
                ...
                />
                <Label htmlFor="yoga_stil_bikram" className="font-normal">Bikram Yoga</Label>
              </div>
            </div>
          </div>
          <div className="space-y-2">
            <Label htmlFor="biografie">Kurzbeschreibung / Biografie</Label>
            <Textarea
              id="biografie"
