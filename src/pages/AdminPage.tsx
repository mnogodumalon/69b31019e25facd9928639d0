...
const KURSLEITERVERWALTUNG_FIELDS = [
  ...
  { key: 'yoga_stil', label: 'Yoga-Spezialisierungen', type: 'multiplelookup/checkbox', options: [{ key: 'hatha', label: 'Hatha Yoga' }, { key: 'vinyasa', label: 'Vinyasa Yoga' }, { key: 'yin', label: 'Yin Yoga' }, { key: 'ashtanga', label: 'Ashtanga Yoga' }, { key: 'kundalini', label: 'Kundalini Yoga' }, { key: 'restorative', label: 'Restorative Yoga' }, { key: 'power', label: 'Power Yoga' }, { key: 'bikram', label: 'Bikram Yoga' }] },
  { key: 'biografie', label: 'Kurzbeschreibung / Biografie', type: 'string/textarea' },
...
const KURSVERWALTUNG_FIELDS = [
  ...
  { key: 'kursbeschreibung', label: 'Kursbeschreibung', type: 'string/textarea' },
  { key: 'yoga_stil_kurs', label: 'Yoga-Stil', type: 'lookup/select', options: [{ key: 'vinyasa', label: 'Vinyasa Yoga' }, { key: 'hatha', label: 'Hatha Yoga' }, { key: 'yin', label: 'Yin Yoga' }, { key: 'ashtanga', label: 'Ashtanga Yoga' }, { key: 'kundalini', label: 'Kundalini Yoga' }, { key: 'restorative', label: 'Restorative Yoga' }, { key: 'power', label: 'Power Yoga' }, { key: 'bikram', label: 'Bikram Yoga' }] },
  { key: 'niveau', label: 'Kursniveau', type: 'lookup/radio', options: [{ key: 'anfaenger', label: 'Anfänger' }, { key: 'mittelstufe', label: 'Mittelstufe' }, { key: 'fortgeschrittene', label: 'Fortgeschrittene' }, { key: 'alle', label: 'Alle Niveaus' }] },
  { key: 'kursleiter', label: 'Kursleiter', type: 'applookup/select', targetEntity: 'kursleiter_verwaltung', targetAppId: 'KURSLEITER_VERWALTUNG', displayField: 'vorname' },
...
const TEILNEHMERANMELDUNG_FIELDS = [
  { key: 'gesundheitshinweise', label: 'Gesundheitliche Hinweise / Anmerkungen', type: 'string/textarea' },
  { key: 'anmeldedatum', label: 'Anmeldedatum', type: 'date/date' },
  { key: 'einverstaendnis', label: 'Ich stimme der Teilnahme und den Kursbedingungen zu', type: 'bool' },
  { key: 'teilnehmer_vorname', label: 'Vorname', type: 'string/text' },
  { key: 'teilnehmer_nachname', label: 'Nachname', type: 'string/text' },
  { key: 'teilnehmer_email', label: 'E-Mail-Adresse', type: 'string/email' },
  { key: 'teilnehmer_telefon', label: 'Telefonnummer', type: 'string/tel' },
  { key: 'geburtsdatum', label: 'Geburtsdatum', type: 'date/date' },
  { key: 'kurs', label: 'Kurs', type: 'applookup/select', targetEntity: 'kurs_verwaltung', targetAppId: 'KURS_VERWALTUNG', displayField: 'kursname' },
  { key: 'erfahrungslevel', label: 'Dein Erfahrungslevel', type: 'lookup/radio', options: [{ key: 'anfaenger', label: 'Anfänger' }, { key: 'mittelstufe', label: 'Mittelstufe' }, { key: 'fortgeschrittene', label: 'Fortgeschrittene' }] },
];
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('kursleiter_verwaltung');
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      ...
      case 'teilnehmer_anmeldung':
        lists.kurs_verwaltungList = (data as any).kursVerwaltung ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'kurs_verwaltung' && fieldKey === 'kursleiter') {
      const match = (lists.kursleiter_verwaltungList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'teilnehmer_anmeldung' && fieldKey === 'kurs') {
      const match = (lists.kurs_verwaltungList ?? []).find((r: any) => r.record_id === id);
    ...
    >
      ...
      {(createEntity === 'kurs_verwaltung' || dialogState?.entity === 'kurs_verwaltung') && (
        <KursVerwaltungDialog
          ...
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kurs_verwaltung' ? handleUpdate : (fields: any) => handleCreate('kurs_verwaltung', fields)}
          defaultValues={dialogState?.entity === 'kurs_verwaltung' ? dialogState.record?.fields : undefined}
