...
export default function KursleiterVerwaltungPage() {
  const [records, setRecords] = useState<KursleiterVerwaltung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<KursleiterVerwaltung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<KursleiterVerwaltung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<KursleiterVerwaltung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getKursleiterVerwaltung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: KursleiterVerwaltung['fields']) {
    await LivingAppsService.createKursleiterVerwaltungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: KursleiterVerwaltung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateKursleiterVerwaltungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteKursleiterVerwaltungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Kursleiter-Verwaltung"
      subtitle={`${records.length} Kursleiter-Verwaltung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Kursleiter-Verwaltung suchen..."
          value={search}
            ...
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('yoga_stil')}>
                <span className="inline-flex items-center gap-1">
                  ...
                  {sortKey === 'yoga_stil' ? (sortDir === 'asc' ? <ArrowUp size={14} /> : <ArrowDown size={14} />) : <ArrowUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('biografie')}>
                <span className="inline-flex items-center gap-1">
                  Kurzbeschreibung / Biografie
                  {sortKey === 'biografie' ? (sortDir === 'asc' ? <ArrowUp size={14} /> : <ArrowDown size={14} />) : <ArrowUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{Array.isArray(record.fields.yoga_stil) ? record.fields.yoga_stil.map((v: any) => v?.label ?? v).join(', ') : '—'}</TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.biografie ?? '—'}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Kursleiter-Verwaltung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Kursleiter-Verwaltung löschen"
